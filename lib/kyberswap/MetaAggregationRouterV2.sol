...
contract MetaAggregationRouterV2 is Permitable, Ownable {
        ...
        returns (uint256 returnAmount, uint256 gasUsed)
    {
        ...
        if (!_flagsChecked(desc.flags, _FEE_ON_DST)) {
            ...
            (spentAmount, returnAmount) = _executeSwap(
                execution.callTarget, execution.targetData, desc, _isETH(desc.srcToken) ? desc.amount : 0, dstReceiver
            );
        } else {
            bool collected;
            if (!_isETH(desc.srcToken) && _flagsChecked(desc.flags, _SHOULD_CLAIM)) {
            ...
            (spentAmount, returnAmount) = _executeSwap(
                execution.callTarget, execution.targetData, desc, _isETH(desc.srcToken) ? msg.value : 0, address(this)
            );
            {
    ...
    function _collectTokenIfNeeded(SwapDescriptionV2 memory desc, address from, address to)
        internal
        returns (bool collected, uint256 amount)
    {
    ...
    function _checkReturnAmount(uint256 spentAmount, uint256 returnAmount, SwapDescriptionV2 memory desc)
        internal
        pure
    {
